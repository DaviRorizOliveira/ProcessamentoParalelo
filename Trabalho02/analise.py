...
def escreveTabelas(vetorDf) :
    # Verificação de diretório removida
...
def Tabelas() :
    ...
    # Seleção de colunas atualizada para incluir MPI
    dfTempo = df.loc[:,['tamMatriz','tempoSequencial','tempo2Thread','tempo4Thread','tempo8Thread', 'tempoMPI']]
    dfSpeedup = df.loc[:,['tamMatriz','speedup2Thread','speedup4Thread','speedup8Thread', 'speedupMPI']]
    dfEficiencia = df.loc[:,['tamMatriz','eficiencia2Thread','eficiencia4Thread','eficiencia8Thread', 'eficienciaMPI']]
...
# ============ GRÁFICO 1: Tempos de Execução ============
    # Inicia uma nova figura para este gráfico
    ...
    # Adiciona a linha para MPI
    plt.plot(dfTempo['tamMatriz'], dfTempo['tempoMPI'], 'x-', markersize=6, label='MPI', color='#9467bd')


    plt.xticks(dfTempo['tamMatriz'], [f"{int(x)}" for x in dfTempo['tamMatriz']])
    
    # Remove a definição manual de plt.yticks() para permitir que o matplotlib
    # ajuste a escala automaticamente, o que é melhor para dados variáveis.
 ...
 # ========== GRÁFICO 2: SPEEDUP ==========
    # Inicia uma nova figura para este gráfico (corrigido do plt.subplot)
    plt.figure(figsize = (12, 8))
    ...
    # Adiciona a linha para MPI
    plt.plot(dfSpeedup['tamMatriz'], dfSpeedup['speedupMPI'], 'x-', markersize=6, label='MPI', color='#9467bd')
    ...
    plt.figure(figsize=(12, 8)) # Ajustado o tamanho
    ...
    # Adiciona a linha para MPI
    plt.plot(dfEficiencia['tamMatriz'], dfEficiencia['eficienciaMPI'], 'x-', markersize=6, label='MPI', color='#9467bd')
